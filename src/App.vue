...
const tab = ref<'today' | 'add' | 'activity' | 'history' | 'stats' | 'body' | 'settings'>('today')
...
function localDateOffset(daysAgo: number) { const date = new Date(`${today}T12:00:00`); date.setDate(date.getDate() - daysAgo); return new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Bangkok' }).format(date) }
const lastSevenDays = computed(() => Array.from({ length: 7 }, (_, index) => localDateOffset(6 - index)))
const dailyCalories = (date: string) => foods.value.filter((food) => food.date === date).reduce((sum, food) => sum + food.calories, 0)
...
const goalDays = computed(() => historyDays.value.filter((date) => {
  const calories = dailyCalories(date)
  return calories >= targets.value.calories * 0.9 && calories <= targets.value.calories * 1.1
}).length)
const currentStreak = computed(() => {
  let count = 0
  for (let offset = 0; ; offset += 1) {
    const date = localDateOffset(offset); const calories = dailyCalories(date)
    if (calories < targets.value.calories * 0.9 || calories > targets.value.calories * 1.1) break
    count += 1
  }
  return count
})
const bestStreak = computed(() => {
  const dates = [...historyDays.value].sort(); let best = 0; let current = 0; let prior = ''
  for (const date of dates) {
    const calories = dailyCalories(date); const metGoal = calories >= targets.value.calories * 0.9 && calories <= targets.value.calories * 1.1
    const consecutive = !prior || (new Date(`${date}T12:00:00`).getTime() - new Date(`${prior}T12:00:00`).getTime()) === 86400000
    current = metGoal && consecutive ? current + 1 : metGoal ? 1 : 0
    best = Math.max(best, current); prior = date
  }
  return best
})
...
async function estimateFood() {
  if (!form.value.name.trim()) { error.value = 'กรุณาพิมพ์ชื่ออาหารก่อนประเมิน'; return }
  isEstimating.value = true; error.value = ''
  try {
    const response = await fetch('/api/estimate-food', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description: form.value.name }) })
    const result = await response.json() as Macro & { error?: string }
    if (!response.ok) throw new Error(result.error || 'ไม่สามารถประเมินอาหารได้')
    form.value.calories = String(result.calories); form.value.protein = String(result.protein); form.value.carbs = String(result.carbs); form.value.fat = String(result.fat)
    error.value = 'AI ประเมินแล้ว โปรดตรวจและแก้ไขตัวเลขก่อนบันทึก'
  } catch (reason) { error.value = reason instanceof Error ? reason.message : 'ไม่สามารถประเมินอาหารได้' }
  finally { isEstimating.value = false }
}
    ...
    <template v-else>
      ...
      <section v-if="tab === 'today'" class="px-5">
        ...
        <div v-else class="mt-3 space-y-2">
          <article v-for="food in foods.filter(f => f.date === today)" :key="food.id" class="flex items-center rounded-2xl bg-white p-4 shadow-sm">
            <div class="flex-1"><p class="font-semibold">{{ food.name }}</p><p class="text-sm text-slate-500">{{ { breakfast: 'เช้า', lunch: 'กลางวัน', dinner: 'เย็น', snack: 'ของว่าง' }[food.meal] }} · P {{ food.protein }}g · C {{ food.carbs }}g · F {{ food.fat }}g</p></div>
            <div class="text-right"><p class="font-bold">{{ food.calories }} kcal</p><div class="mt-1 flex justify-end gap-3"><button class="text-xs text-teal-700" @click="editFood(food)">แก้ไข</button><button class="text-xs text-red-500" @click="removeFood(food.id)">ลบ</button></div></div>
          </article>
        </div>
      </section>
      <section v-else-if="tab === 'add'" class="px-5"><div class="flex items-center gap-3"><button class="text-2xl" @click="resetFoodForm(); tab = 'today'">‹</button><h1 class="text-2xl font-bold">{{ editingFoodId ? 'แก้ไขอาหาร' : 'เพิ่มอาหาร' }}</h1></div><p class="mt-2 text-slate-600">พิมพ์รายละเอียดอาหารให้ AI ประเมิน แล้วตรวจแก้ก่อนบันทึก</p><form class="mt-6 space-y-4" @submit.prevent="saveFood"><input v-model="form.name" :disabled="isEstimating" placeholder="เช่น กะเพราไก่ไข่ดาว 1 จาน" class="w-full rounded-2xl border-0 bg-white px-4 py-4 shadow-sm outline-teal-600 disabled:opacity-60"><button v-if="!editingFoodId" type="button" :disabled="isEstimating" class="w-full rounded-2xl border border-teal-200 bg-teal-50 py-3 font-semibold text-teal-800 disabled:opacity-60" @click="estimateFood">{{ isEstimating ? 'AI กำลังประเมิน…' : '✦ ประเมินด้วย AI' }}</button><select v-model="form.meal" class="w-full rounded-2xl bg-white px-4 py-4 shadow-sm outline-teal-600"><option value="breakfast">อาหารเช้า</option><option value="lunch">อาหารกลางวัน</option><option value="dinner">อาหารเย็น</option><option value="snack">ของว่าง</option></select><div class="grid grid-cols-2 gap-3"><label v-for="item in [{key:'calories',label:'พลังงาน (kcal)'}, {key:'protein',label:'โปรตีน (g)'}, {key:'carbs',label:'คาร์บ (g)'}, {key:'fat',label:'ไขมัน (g)'}]" :key="item.key"><span class="mb-1 block text-sm text-slate-600">{{ item.label }}</span><input v-model="form[item.key as keyof typeof form]" type="number" min="0" inputmode="decimal" class="w-full rounded-xl bg-white px-3 py-3 shadow-sm outline-teal-600"></label></div><p v-if="error" class="text-sm" :class="error.includes('AI ประเมินแล้ว') ? 'text-amber-700' : 'text-red-600'">{{ error }}</p><button :disabled="isEstimating" class="w-full rounded-2xl bg-teal-700 py-4 font-semibold text-white disabled:opacity-60">{{ editingFoodId ? 'บันทึกการแก้ไข' : 'บันทึกอาหาร' }}</button></form></section>
      ...
      <section v-else-if="tab === 'stats'" class="px-5"><h1 class="text-2xl font-bold">สถิติ</h1><p class="mt-2 text-slate-600">ภาพรวมการบันทึก 7 วันล่าสุด</p><div class="mt-6 grid grid-cols-2 gap-3"><div class="rounded-2xl bg-teal-700 p-4 text-white"><p class="text-sm text-teal-100">เฉลี่ยต่อวัน</p><p class="mt-1 text-2xl font-bold">{{ weeklyAverage.toLocaleString() }} <span class="text-sm font-normal">kcal</span></p></div><div class="rounded-2xl bg-white p-4 shadow-sm"><p class="text-sm text-slate-500">รวม 7 วัน</p><p class="mt-1 text-2xl font-bold">{{ weeklyCalories.toLocaleString() }} <span class="text-sm font-normal">kcal</span></p></div><div class="rounded-2xl bg-white p-4 shadow-sm"><p class="text-sm text-slate-500">ทำเป้าสำเร็จ</p><p class="mt-1 text-2xl font-bold">{{ goalDays }} <span class="text-sm font-normal">วัน</span></p></div><div class="rounded-2xl bg-white p-4 shadow-sm"><p class="text-sm text-slate-500">Streak ปัจจุบัน</p><p class="mt-1 text-2xl font-bold">{{ currentStreak }} <span class="text-sm font-normal">วัน</span></p></div></div><div class="mt-7 rounded-3xl bg-white p-5 shadow-sm"><div class="flex items-baseline justify-between"><h2 class="font-bold">แคลอรีรายวัน</h2><p class="text-xs text-slate-500">เป้า {{ targets.calories }} kcal</p></div><div class="mt-5 flex h-40 items-end justify-between gap-2"><div v-for="date in lastSevenDays" :key="date" class="flex h-full flex-1 flex-col justify-end"><p class="mb-1 text-center text-xs font-medium">{{ dailyCalories(date) || '-' }}</p><div class="rounded-t-lg bg-teal-600" :style="{ height: `${Math.min(100, dailyCalories(date) / Math.max(targets.calories, 1) * 100)}%`, minHeight: dailyCalories(date) ? '4px' : '0' }"/><p class="mt-2 text-center text-[10px] text-slate-500">{{ formatHistoryDate(date).split(' ')[1] }}</p></div></div></div><div class="mt-5 rounded-2xl bg-amber-50 p-4 text-amber-950"><p class="font-semibold">Best streak: {{ bestStreak }} วัน</p><p class="mt-1 text-sm">นับวันที่พลังงานอยู่ในช่วง 90–110% ของเป้าหมาย</p></div></section>
      ...
      <nav class="fixed bottom-0 left-1/2 flex w-full max-w-md -translate-x-1/2 justify-around border-t bg-white px-2 py-3"><button :class="tab === 'today' ? 'text-teal-700' : 'text-slate-500'" @click="tab = 'today'">⌂<span class="block text-[10px]">วันนี้</span></button><button :class="tab === 'history' ? 'text-teal-700' : 'text-slate-500'" @click="tab = 'history'">◷<span class="block text-[10px]">ประวัติ</span></button><button class="-mt-8 grid h-14 w-14 place-items-center rounded-full bg-teal-700 text-3xl text-white shadow-lg" @click="tab = 'add'">+</button><button :class="tab === 'stats' ? 'text-teal-700' : 'text-slate-500'" @click="tab = 'stats'">▥<span class="block text-[10px]">สถิติ</span></button><button :class="tab === 'body' ? 'text-teal-700' : 'text-slate-500'" @click="tab = 'body'">♙<span class="block text-[10px]">ร่างกาย</span></button><button :class="tab === 'settings' ? 'text-teal-700' : 'text-slate-500'" @click="tab = 'settings'">⚙<span class="block text-[10px]">ตั้งค่า</span></button></nav>
    </template>
  </main>
</template>
