...
function estimateFood() {
  ...
  const estimate = estimateNutrition(form.value.name)
  form.value.calories = String(estimate.calories); form.value.protein = String(estimate.protein); form.value.carbs = String(estimate.carbs); form.value.fat = String(estimate.fat)
  error.value = estimate.matched ? 'ใส่ค่าประมาณแล้ว โปรดตรวจและแก้ไขก่อนบันทึก' : 'ไม่พบเมนูที่ตรงกัน จึงใส่ค่าประมาณทั่วไป โปรดแก้ไขก่อนบันทึก'
}
...
<template>
  <main class="mx-auto min-h-screen max-w-md bg-stone-50 pb-24">
    ...
    <template v-else>
      ...
      <section v-else-if="tab === 'add'" class="px-5"><div class="flex items-center gap-3"><button class="text-2xl" @click="resetFoodForm(); tab = 'today'">‹</button><h1 class="text-2xl font-bold">{{ editingFoodId ? 'แก้ไขอาหาร' : 'เพิ่มอาหาร' }}</h1></div><p class="mt-2 text-slate-600">พิมพ์ชื่ออาหารเพื่อประเมิน หรือกรอกตัวเลขเอง</p><form class="mt-6 space-y-4" @submit.prevent="saveFood"><input v-model="form.name" placeholder="เช่น กะเพราไก่ไข่ดาว 1 จาน" class="w-full rounded-2xl border-0 bg-white px-4 py-4 shadow-sm outline-teal-600"><button v-if="!editingFoodId" type="button" class="w-full rounded-2xl border border-teal-200 bg-teal-50 py-3 font-semibold text-teal-800" @click="estimateFood">✦ ประเมินโภชนาการ</button><select v-model="form.meal" class="w-full rounded-2xl bg-white px-4 py-4 shadow-sm outline-teal-600"><option value="breakfast">อาหารเช้า</option><option value="lunch">อาหารกลางวัน</option><option value="dinner">อาหารเย็น</option><option value="snack">ของว่าง</option></select><div class="grid grid-cols-2 gap-3"><label v-for="item in [{key:'calories',label:'พลังงาน (kcal)'}, {key:'protein',label:'โปรตีน (g)'}, {key:'carbs',label:'คาร์บ (g)'}, {key:'fat',label:'ไขมัน (g)'}]" :key="item.key"><span class="mb-1 block text-sm text-slate-600">{{ item.label }}</span><input v-model="form[item.key as keyof typeof form]" type="number" min="0" inputmode="decimal" class="w-full rounded-xl bg-white px-3 py-3 shadow-sm outline-teal-600"></label></div><p v-if="error" class="text-sm" :class="error.includes('ประเมิน') ? 'text-amber-700' : 'text-red-600'">{{ error }}</p><button class="w-full rounded-2xl bg-teal-700 py-4 font-semibold text-white">{{ editingFoodId ? 'บันทึกการแก้ไข' : 'บันทึกอาหาร' }}</button></form></section>
