...
import type { FoodLog, Macro, Targets, User } from './types'

const user = ref<User | null>(store.getUser())
const tab = ref<'today' | 'add' | 'settings'>('today')
...
const remaining = computed<Macro>(() => ({
  calories: targets.value.calories - totals.value.calories, protein: targets.value.protein - totals.value.protein,
...
function saveFood() {
  ...
  const food: FoodLog = { id: crypto.randomUUID(), name: value.name.trim(), meal: value.meal, date: today, createdAt: new Date().toISOString(), calories: Number(value.calories) || 0, protein: Number(value.protein) || 0, carbs: Number(value.carbs) || 0, fat: Number(value.fat) || 0 }
  store.saveFood(food); foods.value = store.getFoods(); form.value = { name: '', meal: 'lunch', calories: '', protein: '', carbs: '', fat: '' }; error.value = ''; tab.value = 'today'
}
